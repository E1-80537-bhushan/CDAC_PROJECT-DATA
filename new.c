...
typedef struct review {
 ...
 char review[100];
 int rating;
 time_t timestamp;  //time_t available in time.h directory
}review;

typedef struct user
{
 int user_id;
 char f_name[20]; //1 name
...
typedef struct movies {  //stuct for movie
 int id;
 char title[50];
 char releasedate[50];
}movies;

void sign_in(user *u);  //pointer u for user  
void sign_up(user *u);  
...
void displayreview(review *r);  //pointer r for review struct
...
int u_id = 0; //global variabble for user
struct review re;
review reviewsArray[100]; 
int numReviews = 0;//
int r_id= 0;//g for review

int main()
{
 int choice;
 user u[20];//user structure variable u size 20
...
void sign_in(user *u)
{
 ...
 user temp;//take temp var
 int temp_id;//for temp id store 
 
 FILE *fptr = fopen("user_data.txt", "a+");//a+ we write us //a+=append to add after content file
 if(fptr == NULL)
 {
...
int change_password(user *u,int id)//struct pass update
{
 FILE *file;
 int userIdToEdit = id;
...
void password(user *u)//file password update 
{
...
void displayMyReview(review *reviews, int numReviews, int MyId) 
{
 ...
 // printf("user id=%d\n",reviews[i].user_id);
 // printf("my id=%d\n",MyId);
 if (reviews[i].user_id == MyId) //own rev id comp then print
 {
 printf("\n");
 printf("Review ID: %d\n", reviews[i].review_id);
 printf("Movie ID: %d\n", reviews[i].movie_id);
